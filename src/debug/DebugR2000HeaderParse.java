...
/**
 * Debug R2000 header parsing byte-by-byte
 */
public class DebugR2000HeaderParse {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("=== R2000 Header Parsing Debug (Arc.dwg) ===");
        System.out.println("File size: " + fileData.length + " bytes\n");

        // 1. Version string (6 bytes): AC1015
        ...
        System.out.printf("\nCurrent position: bit %d (byte 0x%X, offset %d.%d)\n",
            currentPos, currentPos / 8, currentPos / 8, currentPos % 8);

        // 9. Read section locators
        System.out.println("\n9. Section locators (" + sectionCount + " total, 12 bytes each):");
        for (int i = 0; i < sectionCount && i < 10; i++) {  // Limit to first 10 to avoid spam
            ...
            System.out.printf("   Locator[%d]:\n", i);
            System.out.printf("     position: 0x%X\n", posBefore / 8);
            System.out.printf("     record_number: 0x%X (%d)\n", number, number);
            System.out.printf("     seeker: 0x%X (%d)\n", address, address);
        ...
        // Show position where Objects section should start
        long objectsStartPos = input.position() / 8;
        System.out.printf("\nObjects section should start at byte offset: 0x%X (%d)\n",
            objectsStartPos, objectsStartPos);
    }
}
