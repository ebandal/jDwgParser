...
import structure.entities.DwgDimensionLinear;
...
public class DimensionLinearObjectReader implements ObjectReader {
    ...
    public int objectTypeCode() {
        return DwgObjectType.DIMENSION_LINEAR.typeCode();
    }

    @Override
    public void read(DwgObject target, byte[] data, int offset, DwgVersion version) throws Exception {
        if (!(target instanceof DwgDimensionLinear)) return;
        ...
        if (byteOff + 8 <= data.length) {
            double hdir = readBitDouble(data, byteOff);
            dim.setHorizontalDirection(hdir);
            byteOff += 8;
        }
