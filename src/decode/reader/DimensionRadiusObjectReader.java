...
import structure.entities.DwgDimensionRadius;
import structure.entities.DwgObject;
import structure.entities.DwgObjectType;
import structure.entities.Point3D;

public class DimensionRadiusObjectReader implements ObjectReader {

    @Override
    public int objectTypeCode() {
        return DwgObjectType.DIMENSION_RADIUS.typeCode();
    }

    @Override
    public void read(DwgObject target, byte[] data, int offset, DwgVersion version) throws Exception {
        if (!(target instanceof DwgDimensionRadius)) return;
        DwgDimensionRadius dim = (DwgDimensionRadius) target;
