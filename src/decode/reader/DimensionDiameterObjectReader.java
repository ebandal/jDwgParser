...
import structure.entities.DwgDimensionDiameter;
import structure.entities.DwgObject;
import structure.entities.DwgObjectType;
import structure.entities.Point3D;

public class DimensionDiameterObjectReader implements ObjectReader {

    @Override
    public int objectTypeCode() {
        return DwgObjectType.DIMENSION_DIAMETER.typeCode();
    }

    @Override
    public void read(DwgObject target, byte[] data, int offset, DwgVersion version) throws Exception {
        if (!(target instanceof DwgDimensionDiameter)) return;
        DwgDimensionDiameter dim = (DwgDimensionDiameter) target;
