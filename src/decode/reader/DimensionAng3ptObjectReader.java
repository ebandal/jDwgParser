...
import structure.entities.DwgDimensionAng3pt;
import structure.entities.DwgObject;
import structure.entities.DwgObjectType;
import structure.entities.Point3D;

public class DimensionAng3ptObjectReader implements ObjectReader {

    @Override
    public int objectTypeCode() {
        return DwgObjectType.DIMENSION_ANG_3PT.typeCode();
    }

    @Override
    public void read(DwgObject target, byte[] data, int offset, DwgVersion version) throws Exception {
        if (!(target instanceof DwgDimensionAng3pt)) return;
        DwgDimensionAng3pt dim = (DwgDimensionAng3pt) target;
